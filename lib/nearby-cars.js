...
const NEARBY_CAR_ROUTE_ATTEMPTS = 6
...
function buildFallbackCarRoute(centerLat, centerLng) {
  const legs = []
  const legsCount = 5
  for (let i = 0; i < legsCount; i += 1) {
    const startBearing =
      (360 / legsCount) * i + (Math.random() * 40 - 20)
    const endBearing =
      (360 / legsCount) * (i + 1) + (Math.random() * 40 - 20)
    const distance =
      NEARBY_CAR_MIN_DISTANCE_KM +
      Math.random() * (NEARBY_CAR_MAX_DISTANCE_KM - NEARBY_CAR_MIN_DISTANCE_KM)
    legs.push({
      start: getDestinationPoint(centerLat, centerLng, startBearing, distance),
      end: getDestinationPoint(centerLat, centerLng, endBearing, distance),
    })
  }
  const coords = []
  legs.forEach((leg) => {
    coords.push([leg.start.lat, leg.start.lng], [leg.end.lat, leg.end.lng])
  })
  return interpolateRoute(coords, 0.03)
}
...
async function buildNearbyCarRoute(centerLat, centerLng) {
  for (let attempt = 0; attempt < NEARBY_CAR_ROUTE_ATTEMPTS; attempt += 1) {
    ...
    let coordinates = await fetchCarRoute(start.lat, start.lng, end.lat, end.lng)
    if (!coordinates || coordinates.length === 0) {
      // Retry once with a short delay (the public OSRM demo server is
      // rate-limited, so a quiet retry keeps the cars on screen).
      await new Promise((resolve) => setTimeout(resolve, 700 + Math.random() * 600))
      coordinates = await fetchCarRoute(start.lat, start.lng, end.lat, end.lng)
    }
  ...
  return buildFallbackCarRoute(centerLat, centerLng)
}
