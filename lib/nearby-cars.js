...
function startNearbyCarOnRoute(map, L, routeCoords, index) {
  ...
  const tick = () => {
    ...
    // Any stop (route end or stop sign) clears once its pause has elapsed.
    if (stopped && nowTs >= stopUntil) {
      stopped = false
      if (posMeters >= totalMeters || posMeters <= 0) {
        // Turn around at a route end: nudge just inside so the next pulse
        // drives away instead of being clamped back onto the end forever.
        direction = direction > 0 ? -1 : 1
        posMeters = direction > 0 ? 0.5 : totalMeters - 0.5
      }
    }
