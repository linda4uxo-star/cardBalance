      ...
      fetch(url, { signal: controller.signal })
        ...
        .then((data) => {
          ...
          if (coordinates && coordinates.length > 0) {
            ...
            resolve(coordinates.map(([lng, lat]) => [lat, lng]))
            return
          }
          if (++settled === endpoints.length) resolve(null)
        })
        .catch(() => {
          clearTimeout(timeoutId)
          if (++settled === endpoints.length) resolve(null)
...
function startNearbyCarOnRoute(map, L, routeCoords, index) {
  ...
  // Smooth acceleration but firmer braking — the red brake lamps light up
  // while the car is shedding speed.
  const accel = targetSpeed > speedKmh ? 7 : 15
